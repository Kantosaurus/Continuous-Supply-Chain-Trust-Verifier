...
async fn test_find_users_by_tenant() {
    ...
    user_repo.create(&user2).await.expect("Failed to create user2");
...
async fn test_update_user() {
    ...
    user_repo.update(&user).await.expect("Failed to update user");
...
async fn test_count_by_tenant() {
    ...
    // Create users
    user_repo.create(&create_test_user(tenant.id)).await.unwrap();
    user_repo.create(&create_test_user(tenant.id)).await.unwrap();
    user_repo.create(&create_test_user(tenant.id)).await.unwrap();
